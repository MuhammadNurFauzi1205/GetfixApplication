...
import com.example.getfixapplication.utils.ConstVal.EXTRA_NAME
...
class LoginActivity : AppCompatActivity() {
    ...
    private fun firebaseAuthWithGugel(idToken: String) {
        ...
        if (user != null){
            val intent = Intent(applicationContext, NavigationHomeActivity::class.java)
            intent.putExtra(ConstVal.EXTRA_NAME, user.displayName)
            startActivity(intent)
            finish()
        }
    }
